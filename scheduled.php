				...
				{				
					...
					if($links_tracking)
					{
						...
						if(strpos($html, '</webversion>')==true || strpos($html, '[webversion]')==true)
							mysqli_query($mysqli, 'INSERT INTO links (campaign_id, link) VALUES ('.$campaign_id.', "'.'http://'.$from_email_domain.'/w/'.short($campaign_id).'")');
						
						//Insert reconsent link
						if(strpos($html, '[reconsent]')==true)
							mysqli_query($mysqli, 'INSERT INTO links (campaign_id, link) VALUES ('.$campaign_id.', "'.'http://'.$from_email_domain.'/r?c='.short($campaign_id).'")');
				    ...
				    while($row = mysqli_fetch_array($r))
				    {
						...
						{			
						    while($row2 = mysqli_fetch_array($r2))
						    {
								...
								{									
									//replace new links on HTML code
							    	$html_treated = str_replace('href="'.$link.'"', 'href="'.'http://'.$from_email_domain.'/l/'.short($subscriber_id).'/'.short($linkID).'/'.short($campaign_id).'"', $html_treated);
							    	$html_treated = str_replace('href=\''.$link.'\'', 'href="'.'http://'.$from_email_domain.'/l/'.short($subscriber_id).'/'.short($linkID).'/'.short($campaign_id).'"', $html_treated);
				    	...
				    	$html_treated = str_replace('<webversion', '<a href="'.'http://'.$from_email_domain.'/w/'.short($subscriber_id).'/'.short($subscriber_list).'/'.short($campaign_id).'" ', $html_treated);
				    	...
				    	$html_treated = str_replace('<unsubscribe', '<a href="'.'http://'.$from_email_domain.'/unsubscribe/'.short($email).'/'.short($subscriber_list).'/'.short($campaign_id).'" ', $html_treated);
				    	...
				    	if($opens_tracking)
				    	{
							...
							$html_treated .= '<img src="'.'http://'.$from_email_domain.'/t/'.short($campaign_id).'/'.short($subscriber_id).'" alt="" style="width:1px;height:1px;"/>';
						}
